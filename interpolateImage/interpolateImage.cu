...
__global__ void NearestInterpolateKernel(const float* const imageIn,
										float* const out,
										const size_t height,
										const size_t width,
										const float* const x,
                 ...
                 int nrhs, mxArray const *prhs[])
{
	...
	for(size_t i = 0; i < imageDepth; i++){
		...
		NearestInterpolateKernel<<<gridSize(numPoints), BLOCK_SIZE>>>(imageLayerPtr, outLayerPtr, imageHeight, imageWidth, xPtr, yPtr, numPoints);
		CudaCheckError();
	}
